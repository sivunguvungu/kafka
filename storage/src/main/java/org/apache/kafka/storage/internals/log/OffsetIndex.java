 ...
 * segment. This index may be sparse: that is it may not hold an entry for all
 * messages in the log.
 *
 * The index is stored in a file that is pre-allocated to hold a fixed maximum
 * number of 8-byte entries.
 *
 * The index supports lookups against a memory-map of this file. These lookups
 * are done using a simple binary search variant to locate the offset/location
 * pair for the greatest offset less than or equal to the target offset.
 *
 * Index files can be opened in two ways: either as an empty, mutable index that
 * allows appends or an immutable read-only index file that has previously been
 * populated. The makeReadOnly method will turn a mutable file into an immutable
 * one and truncate off any extra bytes. This is done when the index file is
 * rolled over.
 *
 * No attempt is made to checksum the contents of this file, in the event of a
 * crash it is rebuilt.
 *
 * The file format is a series of entries. The physical format is a 4 byte
 * "relative" offset and a 4 byte file location for the message with that
 * offset. The offset stored is relative to the base offset of the index file.
 * So, for example, if the base offset was 50, then the offset 55 would be
 * stored as 5. Using relative offsets in this way let's us use only 4 bytes for
 * the offset.
 *
 * The frequency of entries is up to the user of this class.
 *
 * All external APIs translate from relative offsets to full offsets, so users
 * of this class do not interact with the internal storage format.
 */
public final class OffsetIndex extends AbstractIndex {
	...
	public void sanityCheck() {
		if (entries() != 0 && lastOffset < baseOffset())
			throw new CorruptIndexException("Corrupt index found, index file " + file().getAbsolutePath()
					+ " has non-zero size " + "but the last offset is " + lastOffset
					+ " which is less than the base offset " + baseOffset());
	...
	/**
	 * Find the largest offset less than or equal to the given targetOffset and
	 * return a pair holding this offset and its corresponding physical file
	 * position.
	 *
	 * @param targetOffset The offset to look up.
	 * @return The offset found and the corresponding file position for this offset
	 *         If the target offset is smaller than the least entry in the index (or
	 *         the index is empty), the pair (baseOffset, 0) is returned.
	 */
	public OffsetPosition lookup(long targetOffset) {
		return inRemapReadLock(() -> {
			MappedByteBuffer mbuf = mmap();
			if (OperatingSystem.IS_WINDOWS && mbuf == null) {
				return new OffsetPosition(baseOffset(), 0);
			}
			ByteBuffer idx = mbuf.duplicate();
	...
	/**
	 * Find an upper bound offset for the given fetch starting position and size.
	 * This is an offset which is guaranteed to be outside the fetched range, but
		...
		inLock(() -> {
			...
			if (entries() == 0 || offset > lastOffset) {
				...
				throw new InvalidOffsetException("Attempt to append an offset " + offset + " to position " + entries()
						+ " no larger than the last offset appended (" + lastOffset + ") to "
	...
	public void truncateTo(long offset) {
		inLock(() -> {
			...
			if (OperatingSystem.IS_WINDOWS && mbuf == null) {
				...
				/*
				 * There are 3 cases for choosing the new size 1) if there is no entry in the
				 * index <= the offset, delete everything 2) if there is an entry for this exact
				 * offset, delete it and everything larger than it 3) if there is no entry for
				 * this offset, delete everything larger than the next smallest
				 */
